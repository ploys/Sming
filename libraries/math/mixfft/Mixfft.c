...
static void initTrig(int radix, double trigRe[], double trigIm[])
{
    int i;
    double w,xre,xim;
...
static void initTrigAccurate(int radix, double trigRe[], double trigIm[])
{
    int i;
    double w;
...
{   /* twiddleTransf */ 
    ...
    for (dataNo=0; dataNo<sofarRadix; dataNo++)
    {
		...
		switch(radix) {
			case  2:
		        twiddleFFT_2(sofarRadix, radix, remainRadix, groupOffset, 
					         twiddleRe, twiddleIm, yRe, yIm);
				break;
			case  3:  
		        twiddleFFT_3(sofarRadix, radix, remainRadix, groupOffset, 
					         twiddleRe, twiddleIm, yRe, yIm);
				break;
			case  4:
		        twiddleFFT_4(sofarRadix, radix, remainRadix, groupOffset,
							 twiddleRe, twiddleIm, yRe, yIm);
				break;
			case  5:  
		        twiddleFFT_5(sofarRadix, radix, remainRadix, groupOffset,
							 twiddleRe, twiddleIm, yRe, yIm);
				break;
			case  7:
		        twiddleFFT_7(sofarRadix, radix, remainRadix, groupOffset,
							 twiddleRe, twiddleIm, yRe, yIm);
				break;
			case  8:  
		        twiddleFFT_8(sofarRadix, radix, remainRadix, groupOffset,
					         twiddleRe, twiddleIm, yRe, yIm);
				break;
			case 10: 
		        twiddleFFT_10(sofarRadix, radix, remainRadix, groupOffset,
					         twiddleRe, twiddleIm, yRe, yIm);
				break;
			default:  
				twiddleFFT_odd(sofarRadix, radix, remainRadix, groupOffset,
					           twiddleRe, twiddleIm, trigRe, trigIm, yRe, yIm); 
				break;
		}
...
static void twiddleTransfCtxt(int sofarRadix, int radix, int remainRadix,
					   double tblRe[], double tblIm[],
                       double yRe[], double yIm[])
...
static void transf(int sofarRadix, int radix, int remainRadix,
                    double yRe[], double yIm[])
    ...
    for (dataNo=0; dataNo<sofarRadix; dataNo++)
    {
		switch(radix) {
			...
			case 10:
				...
				break;
			default:  
				FFT_odd(sofarRadix, radix, remainRadix, groupOffset,
					    trigRe, trigIm, yRe, yIm); 
				break;
		}
        dataOffset=dataOffset+1;
        groupOffset=dataOffset;
...
/*
	for (count=1; count<=nFactor; count++)
		printf("\ncount = %5d  factor = %5d", count, actualRadix[count]);
*/
	if (nFactor>1)
...
void fftInv(int n, double xRe[], double xIm[], double yRe[], double yIm[])
{
    int   sofarRadix[maxFactorCount], 
          actualRadix[maxFactorCount], 
          remainRadix[maxFactorCount];
...
void fftCtxt(struct fftContext *fc, double xRe[], double xIm[], double yRe[], double yIm[])
{
    int   count;
	...
	if (fc->factorCount>1)
		for (count=2; count<=fc->factorCount; count++)
		  twiddleTransfCtxt(fc->sofarRdx[count], fc->actualRdx[count], fc->remainRdx[count], 
		                    fc->tableRe, fc->tableIm, yRe, yIm);
}   /* fftCtxt */

void fftInvCtxt(struct fftContext *fc, double xRe[], double xIm[], double yRe[], double yIm[])
{
    int   count, i;
	double scaleRe, scaleIm;
...
void fftCtxtIQ(struct fftContext *fc, double xIQ[], double yRe[], double yIm[])
{
    int   count;
...
int createFftContext(int fftLength, struct fftContext *fc)
{
    ...
    fc->tableRe = (double  *) calloc(fftLength, sizeof(double));                        
    fc->tableIm = (double  *) calloc(fftLength, sizeof(double)); 
